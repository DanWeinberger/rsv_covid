...
## US States, most recent 52 weeks
...
last.wk <- max(d1.m.alt$epiwk[d1.m.alt$epiyr==max(d1.m.alt$epiyr)] )
...
cols <- c(rep('gray',3),rgb(1,0,0,0.5),'blue')
...
p1 <- ggplot(df1, aes(
  ...
  labs(title = "Searches for 'RSV' (Google trends), last 52 wks",
       x = element_blank(),
       y = element_blank()) +
  theme(
...
## Selected states, most recent 52 weeks

```{r, fig.width=8, fig.height=6, results=FALSE, message=FALSE, warning=FALSE}
cols <- c(rep('gray',3),rgb(1,0,0,0.3),'blue')
...
Northern Hemisphere shows most recent 52 weeks; Southern Hemisphere shows current calendar year
```{r results=FALSE, message=FALSE, warning=FALSE}
l1b <- country.trends
